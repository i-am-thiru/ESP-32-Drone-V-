...
static void udp_server_rx_task(void *pvParameters)
{
    ...
    while (true) {
        ...
        if (len < 0) {
            ...
            if(rx_buffer[0] == 0x71 && rx_buffer[1] == 0x13 && rx_buffer[2] == 0x01 && rx_buffer[3] == 0x89) // TAKE OFF MODE TO ENABLE
            {
                takeOffMode = false ;//true;
                if(takeOffMode){ 
            ...
            else if(rx_buffer[0] == 0x71 && rx_buffer[1] == 0x13 && rx_buffer[2] == 0x00 && rx_buffer[3] == 0x88) // LAND MODE TO ENABLE
            {
                if(takeOffMode){
                    takeOffMode = false;
                    landMode = true;
            ...
            if(rx_buffer[0] == 0x71 && rx_buffer[1] == 0x13 && rx_buffer[2] == 0x01 && rx_buffer[3] == 0x84){
                if (!isAltHoldEnabled){
                    ...
                    takeoff_done= true;
                    if(altHoldMode){ 
                         targetAltitude = 0.20f ;//distanceDown;
            ...
            {
                if (isAltHoldEnabled && takeoff_done){
                    //altHoldMode = false;
                    targetAltitude = 0.05f; // dks set the target altitude to 0.05 ground
                    printf("althold mode is deactivated %f \n",targetAltitude);
                }
            }
